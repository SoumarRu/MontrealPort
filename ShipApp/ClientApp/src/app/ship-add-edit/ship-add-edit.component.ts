...
export class ShipAddEditComponent implements OnInit {
  ...
  actionType: string;
  formName: string;
  formYearOfConstruction: string;
  formLengthOfVessel: string;
  formVesselWidth: string;
  formGrossTonnage: string;
  formNetTonnage: string;

  errorMessage: any;
  existingShip: Ship;

  constructor(private shipService: ShipService, private formBuilder: FormBuilder, private avRoute: ActivatedRoute, private router: Router) {
    const idParam = 'id';
